import styles from './CommonsBtn.module.scss';
...
const BtnPrimary: React.FC<SubBtnProps> = (props: SubBtnProps) => {
    ...
    return (
        <div className={`${styles.BtnPrimary} ${disabled ? styles.disabled : ''}`} style={width ? { width: `${width}px` } : undefined} onClick={disabled ? undefined : action}>
...
const BtnSecondary: React.FC<SubBtnProps> = (props: SubBtnProps) => {
    const { width, action, content, disabled } = props;
    return (
        <div className={`${styles.btnSecondary} ${disabled ? styles.disabled : ''}`} style={width ? { width: `${width}px` } : undefined} onClick={action}>
            <p className={styles.text}>{content}</p>
        </div>
    );
};
