...
const GoogleConnect: React.FC<GoogleConnectProps> = ({ width, loggedIn }) => {
    ...
    if (loggedIn) {
        return (
            <button className={styles.btnConnected} onClick={() => setShowDisconnect(!showDisconnect)}>
    ...
    return (
        <div className={styles.googleButton} onClick={handleSignIn} style={width ? { width: `${width}px` } : undefined}>
            <Image src={GOOGLE} height={20} width={20} alt="logo-google" /> <span className={styles.text}>Sign up with Google</span>
        </div>
    );
};
