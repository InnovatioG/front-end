...
export default function DraftDashboard({ address }: { address: string | null }) {
    ...
    useEffect(() => {
        ...
        setIsAdmin(isAdmin);
        setAdminView(isAdmin); // Set adminView to true if the user is an admin
        setLoading(false);
    }, [address]);
    ...
    useEffect(() => {
        ...
        const results = campaigns.filter((campaign) => {
            if (isAdmin && adminView) {
                ...
                const userMatches = dataBaseService.getUsers().some((user: User) => user.id === campaign.user_id && user.wallet_address === address);
        ...
        setVisibleCampaigns([]);
    }, [loading, searchTerm, statusContractsFilter, categoryFilter, campaigns, address, isAdmin, adminView, campaignsLoading]);
    ...
    return (
        <div className={styles.draftDashboard}>
            ...
            {visibleCampaigns.length < filteredCampaigns.length && <CommonsBtn type="secondary" action={loadMoreCampaigns} content="Explore more campaigns" width={260} />}
        </div>
    );
}
