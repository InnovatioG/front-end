...
const paramsConnectionS3 = {
    accessKeyId: process.env.NEXT_PUBLIC_AWS_ACCESS_KEY_ID,
    secretAccessKey: process.env.NEXT_PUBLIC_AWS_SECRET_ACCESS_KEY,
    region: process.env.NEXT_PUBLIC_AWS_REGION,
};

AWS.config.update(paramsConnectionS3);
...
export const uploadFileToS3 = async (file: File, bucketName: string, key: string) => {
    ...
    try {
        console.log(`File uploading to S3: ${key} - Params: ${toJson(params)} - Params Connection: ${toJson(paramsConnectionS3)}`);
