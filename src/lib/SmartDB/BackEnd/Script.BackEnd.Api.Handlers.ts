...
export class ScriptApiHandlers extends BaseSmartDBBackEndApiHandlers {
    ...
    public static async restricFilter(user: User | undefined) {
        ...
        //         //const ScriptBackEnd_ = (await import('./protocolBackEnd')).ScriptBackEnd;
        //         const isScriptAdmin = await ScriptBackEndApplied.isAdmin(user.pkh);
        //         if (isScriptAdmin === true) {
        //             const protocols = await ScriptBackEndApplied.getByAdmin(user.pkh, { fieldsForSelect: { _id: true }, loadRelations: {} });
    ...
    //         protocol_id: yup.string().required().label(`Script ID`),
    //     });
    //     //-------------------
